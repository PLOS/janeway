...
class FormDefinition(models.Model):
    ...
    name = models.CharField(
        max_length=255, blank=True, help_text="The name of the form"
    )
    description = models.TextField(blank=True, help_text="Description of the form")
    created_date = models.DateTimeField(
        default=timezone.now, help_text="When the form was created"
    )
...
class FormVariable(models.Model):
    ...
    form_definition = models.ForeignKey(
        FormDefinition, on_delete=models.CASCADE, related_name="variables"
    )
    variable_id = models.CharField(
        blank=True, max_length=255, help_text="The id used to reference this value"
    )
    ...
    value = models.TextField(
        blank=True, null=True, help_text="The starting value for the variable"
    )
...
class FormInput(models.Model):
    ...
    form_definition = models.ForeignKey(
        FormDefinition, on_delete=models.CASCADE, related_name="inputs"
    )
    ...
    enum_choices = models.JSONField(
        null=True, blank=True, help_text="A list of restricted input choices"
    )
    label = models.CharField(
        null=True, max_length=255, blank=True, help_text="The label for the input"
    )
    help_text = models.TextField(
        null=True, blank=True, help_text="The help text for the input"
    )
    value = models.TextField(
        null=True, blank=True, help_text="Allows the user to set a value for the input"
    )
    ...
    placeholder_value = models.CharField(
        null=True,
        max_length=255,
        blank=True,
    ...
    variable_to_save_as = models.CharField(
        null=True, max_length=255, help_text="The name of the variable to save as"
    )

    # Logic and validation fields stored as JSON
    logic = models.JSONField(
        blank=True, null=True, help_text="Conditional logic for the input"
    )
    validation = models.JSONField(
        blank=True, null=True, help_text="Validation rules for the input"
    )
...
class FormValidationRule(models.Model):
    ...
    form_input = models.ForeignKey(
        FormInput, on_delete=models.CASCADE, related_name="validation_rules"
    )
    ...
    error_message = models.TextField(
        blank=True, help_text="Custom error message for when the validation fails"
    )
    condition = models.TextField(
        blank=True, help_text="Optional condition for when the validation should apply"
    )
